...
public class GoogleGenAiConfig {
    @Bean
    @ConditionalOnExpression("#{systemEnvironment.containsKey('AI_MODEL') && systemEnvironment.containsKey('GOOGLE_GENAI_SCOPES') && systemEnvironment['AI_MODEL'] == 'google-genai'}")
    public Client customGoogleGenAiClient() throws IOException {
        GoogleCredentials credentials =
                GoogleCredentials.fromStream(new FileInputStream(System.getenv("GOOGLE_APPLICATION_CREDENTIALS")))
                        .createScoped(System.getenv("GOOGLE_GENAI_SCOPES"));
        return Client.builder()
                .project(System.getenv("CLOUDSDK_CORE_PROJECT"))
                .location(System.getenv("GOOGLE_GENAI_LOCATION"))
                .vertexAI(true)
    ...
    public GoogleGenAiChatModel googleGenAiChatModel(
        ...
        GoogleGenAiChatOptions options = GoogleGenAiChatOptions.builder().model(System.getenv("GOOGLE_GENAI_MODEL")).build();
