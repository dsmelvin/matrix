...
public class TodoWriteTool {
    ...
    private final TodoWriteTool.TodoEventHandler todoListConsumer;

    @FunctionalInterface
    public interface TodoEventHandler {

        void handle(TodoWriteTool.Todos todos);

    }

    protected TodoWriteTool(TodoWriteTool.TodoEventHandler todoListConsumer) {
    ...
    @Tool(name = "TodoWrite", description = """
		...
		**IMPORTANT**: Task descriptions must have two forms:
		...
		""")
    public String todoWrite( @ToolParam(description = "The list of todo items")
								 List<Todos.TodoItem> todos) { // @formatter:on

		validateTodos(new Todos(todos));
		this.todoListConsumer.handle(new Todos(todos));
     ...
     */
    private void validateTodos(TodoWriteTool.Todos todos) {
        if (todos == null || todos.todos() == null) {
            throw new IllegalArgumentException("Todos cannot be null");
        }

        List<TodoWriteTool.Todos.TodoItem> items = todos.todos();

        // Validate each task first (before counting in_progress tasks)
        for (int i = 0; i < items.size(); i++) {
            TodoWriteTool.Todos.TodoItem item = items.get(i);
        ...
        long inProgressCount = items.stream().filter(item -> item.status() == TodoWriteTool.Todos.Status.in_progress).count();
    ...
    public record Todos(@JsonProperty("todos") List<TodoWriteTool.Todos.TodoItem> todos) {
        public record TodoItem(
                @JsonProperty("content") String content,
                @JsonProperty("status") TodoWriteTool.Todos.Status status,
    ...
    public static TodoWriteTool.Builder builder() {
        return new TodoWriteTool.Builder();
    }

    public static class Builder {

        private TodoWriteTool.TodoEventHandler todoEventHandler = todos -> logger.debug("Updated Todos: {}", todos);

        public TodoWriteTool.Builder todoEventHandler(TodoWriteTool.TodoEventHandler todoEventHandler) {
