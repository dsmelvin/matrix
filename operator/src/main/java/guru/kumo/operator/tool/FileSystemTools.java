...
public class FileSystemTools {
        ...
        try {
            ...
            for (Path component : targetAbs) {
                if ("..".equals(component.toString())) {
                    return "Error: Access denied. Path is outside the allowed directories or it is a symbolic link: " + filePath;
                }
            }
	...
	@Tool(name = "Read", description = """
		...
		Usage:
		- The file_path parameter must be an absolute path, not a relative path and it can't be a symbolic link.
    ...
    public String read(
        ...
        try {
            ...
            try {
                MimeType mimeType = MimeTypeUtils.parseMimeType(Files.probeContentType(file.toPath()));
                if (mimeType.equalsTypeAndSubtype(MimeTypeUtils.IMAGE_JPEG) || mimeType.equalsTypeAndSubtype(MimeTypeUtils.IMAGE_PNG)) {
                    return "Can't read images files like PNG or JPEG";
                }
