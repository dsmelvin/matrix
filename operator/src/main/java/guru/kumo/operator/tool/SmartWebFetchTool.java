...
public class SmartWebFetchTool implements AutoCloseable {
    ...
    private static final Duration DEFAULT_CONNECT_TIMEOUT = Duration.ofMinutes(10);
     ...
     */
    private SmartWebFetchTool(ChatClient chatClient, int maxContentLength, boolean domainSafetyCheck, int maxCacheSize, boolean failOpenOnSafetyCheckError, int maxRetries) {
    ...
    public String webFetch(@ToolParam(description = "The URL to fetch content from") String url,
        ...
        if (content != null) {
            log.debug("Cache hit for URL: {} with prompt hash: {}", url, prompt.hashCode());
            return content;
        }

        log.debug("Cache miss for URL: {} with prompt hash: {}", url, prompt.hashCode());
        ...
        while (attempt <= this.maxRetries) {
            try {
                if (attempt > 0) {
                    ...
                    log.debug("Retrying fetch for URL: {} (attempt {}/{}), waiting {}ms", url, attempt,
                ...
                if (response.statusCode() >= 500 && response.statusCode() < 600) {
                    ...
                    log.warn("Fetch attempt {} returned server error {} for URL: {}",
                ...
                log.warn("Fetch attempt {} failed for URL: {}: {}", attempt + 1, url, e.getMessage());
                attempt++;
                ...
                .flatMap(contentType -> {
                    ...
                    if (matcher.find()) {
                        ...
                        try {
                            ...
                            log.warn("Unsupported charset '{}', falling back to UTF-8", charsetName);
                            return Optional.empty();
                        }
                    }
                    return Optional.empty();
    ...
    private String truncate(String content) {
        ...
        if (content.length() > this.maxContentLength) {
            log.warn("Content too long ({} characters). Truncating to {} characters.", content.length(), this.maxContentLength);
    ...
    private void cleanExpiredEntries() {
        ...
        log.debug("Cleaned up expired cache entries. Current cache size: {}", this.urlCache.size());
    }
    ...
    public void close() {
        ...
        this.urlCache.clear();
        log.debug("SmartWebFetchTool closed and resources cleaned up");
    }
    ...
    private static class DefaultDomainCanFetchChecker {
        ...
        public DomainCanFetch check(String url, boolean failOpenOnError) {
            ...
            try {
                ...
                log.warn("Failed to check domain safety for {}: {}", domain, e.getMessage());
    ...
    public static class Builder {
        ...
        private int maxContentLength = 500_000; // Default: 100 KB
