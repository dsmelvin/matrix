...
import guru.kumo.operator.channel.ConsoleChannel;
...
public class OperatorShellCommand {
    ...
    OperatorShellCommand(AgentOperatorService agentOperatorService, ConsoleChannel consoleChannel,
    ...
    public void run(
            @Option(longName = "one-shot", shortName = '1', required = false, description = "One shot execution")
            boolean oneShot,
            @Option(longName = "debug-mode", shortName = 'd', required = false, description = "Turn on debug log level")
            boolean isDebugMode,
        ...
        if (userPrompt != null || userPromptTokens != null) {
            ...
            if (!builder.isEmpty()) {
                messageArrayList.add(UserMessage.builder().text(builder.toString().trim()).build());
            }
        }
        String conversationId = agentOperatorService.processConsoleInitMessage(messageArrayList);
        if (!oneShot) {
            consoleChannel.setConversationId(conversationId);
    ...
    private void debugMode() {
        ...
        context.getLoggerList().forEach(logger -> {
            if (logger.getLevel() != null && !logger.getName().equals("ROOT")) {
                logger.setLevel(null);
            }
        });
        context.getLogger(AgentOperatorService.class).setLevel(Level.DEBUG);
        context.getLogger("ROOT").setLevel(Level.WARN);
        context.getLogger(AgentOperatorService.class.getPackageName().substring(0, AgentOperatorService.class.getPackageName().lastIndexOf("."))).setLevel(Level.INFO);
    }
    ...
    private List<Message> loadSavedSessionMemoryFile(String savedSessionMemoryFileName) {
        try {
            ...
            File savedSessionMemoryFile = new File(Utils.getAbsoluteFilePathName(savedSessionMemoryFileName));
    ...
    private SystemMessage loadSystemPrompt(String systemPromptFileName) {
        try {
            ...
            FileSystemResource operatorSystemPrompt = new FileSystemResource(Utils.getAbsoluteFilePathName(systemPromptFileName));
            if (operatorSystemPrompt.exists() && operatorSystemPrompt.isFile()) {
                ...
                systemPromptEnvMap.put("WORKING_DIRECTORY", workingDirectory);
                systemPromptEnvMap.put("PLATFORM", platform);
                systemPromptEnvMap.put("OS_VERSION", osVersion);
                systemPromptEnvMap.put("TODAY", todayDate);
                systemPromptEnvMap.put("OS_SHELL", System.getenv("SHELL") == null ? "UNKNOWN" : System.getenv("SHELL"));
                systemPromptEnvMap.put("MEMORIES_ROOT_DIERCTORY", sessionMemoryPathName == null ? "NONE" : Utils.getAbsolutePath(sessionMemoryPathName));
    ...
    private UserMessage loadPromptFile(String userPromptFileName) {
        try {
            ...
            File promptFile = new File(Utils.getAbsoluteFilePathName(userPromptFileName));
