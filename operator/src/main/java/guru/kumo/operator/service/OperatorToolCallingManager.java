...
import guru.kumo.operator.tool.ShellTools;
...
public final class OperatorToolCallingManager implements ToolCallingManager {
    ...
    private final AtomicBoolean alwaysRun = new AtomicBoolean(false);
    // @formatter:off
    ...
    OperatorToolCallingManager(Terminal terminal, ObservationRegistry observationRegistry, ToolCallbackResolver toolCallbackResolver,
    ...
    private InternalToolExecutionResult executeToolCall(Prompt prompt, AssistantMessage assistantMessage, ToolContext toolContext) {
        ...
        for (AssistantMessage.ToolCall toolCall : assistantMessage.getToolCalls()) {
            ...
            if (toolName.equals(ShellTools.TOOL_NAME)) {
                BashInput bashInput = jsonMapper.readValue(toolCall.arguments(), BashInput.class);
                if (!alwaysRun.get()) {
                    String nextLine = "";
                    while (nextLine.trim().isEmpty()) {
                        nextLine = reader.readLine(String.format("%n%sAsking permission to run (yes, all or rejected reason):%s%n%s%s%n%n",
                                ColorEnum.BLINK_BOLD_RED, ColorEnum.RESET, ColorEnum.BOLD_HIGH_INTENSITY_RED + bashInput.command(), ColorEnum.RESET));
                    }
                    if (nextLine.equalsIgnoreCase("all")) {
                        alwaysRun.set(true);
                    } else if (!(nextLine.equalsIgnoreCase("t") || nextLine.equalsIgnoreCase("true") || nextLine.equalsIgnoreCase("y") || nextLine.equalsIgnoreCase("yes"))) {
                        toolResponses.add(new ToolResponseMessage.ToolResponse(toolCall.id(), toolName, nextLine.isEmpty() ? "The user rejected to execute this command." : nextLine));
                        continue;
                    }
                }
            }
            ...
            if (limitBreach != null) {
                ...
                if (this.toolCallLimits.onLimitExceeded() == ToolCallLimitBehavior.THROW) {
                    ToolResponseMessage partialToolResponseMessage = ToolResponseMessage.builder().responses(toolResponses).build();
            ...
            Observation parent = ToolCallReactiveContextHolder.getContext().getOrDefault(ObservationThreadLocalAccessor.KEY, this.observationRegistry.getCurrentObservation());
    ...
    public final static class Builder {
        ...
        public OperatorToolCallingManager build() {
            ...
            return new OperatorToolCallingManager(this.terminal, this.observationRegistry, this.toolCallbackResolver, this.toolExecutionExceptionProcessor, toolCallLimits);
        }
    }
}
